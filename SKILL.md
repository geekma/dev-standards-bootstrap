...
3. **写入核心文件**（无冲突时直接写入）：
   ...
   - 复制 `resources/METHODOLOGY.md` -> 目标仓库 `docs/METHODOLOGY.md`（方法论选型总纲，M0-M3 分级唯一权威表）
   - 复制 `resources/methodologies/` 整个目录 -> 目标仓库 `docs/methodologies/`（`development.md` 代码规范 + `data-structures.md` 数据结构规范 + `state-trigger-audit.md` 状态/触发链路审计，v3.4.0）
   - 复制 `resources/templates/bugfix-log.md` -> 目标仓库 `docs/bugfix-log.md`（仓库级 Bug 修复索引，追加式；Bug 修复必须双登记：log 索引行 + CHG 条目，规范 §2.5 阶段 6）
   - 复制 `resources/templates/bug-diagnosis.md`、`bug-impact.md`、`bug-test-plan.md`、`bug-matrix.md`、`bug-config.md`、`bug-tasks.md` -> 目标仓库 `docs/bugs/_templates/`（v3.6.0 新增缺陷文档组、v3.7.0 扩为六件套——改 Bug 一律在 `docs/bugs/<BUG-xxx>/` 新起一组六件套（诊断/影响/测试计划/追踪矩阵/配置DB/任务拆分）并经 `00-governance.json` 的 `bug_ref` 字段与变更关联，`agent-gate begin` 六件全检，只增不改，§2.15 硬性规则 4）
   - 复制 `resources/templates/coding-record.md` -> 变更起编时落目标仓库 `docs/changes/<变更号>/04.5-coding-record.md`（v3.7.0 新增：编码记录，逐文件映射 TASK 编号与改动说明；`agent-gate stop`/`ci` 完工前强检）
   - 复制 `resources/templates/audit-docs-consistency.sh` -> 目标仓库 `tests/audit-docs-consistency.sh` 并 `chmod +x`（**通用层跨文档一致性机器审计**：G1 版本链 / G2 编号连续无跳号 / G3 归档清单与 §3 同源 / G4 bugfix 双登记互证+陈旧占位检测 / G5 RTVM 短引用↔01.5 矩阵回填一致 / G6 最新 CHG §4 必填节完整；文档变更后或接入 CI 运行，失败项即 §2.14 存量回填清单）
4. **询问是否需要可选增强**（不要求默认全装，分别询问）：
   ...
   - 是否要安装**强制执行包**？-> 先展示将写入的文件并确认，再复制下列文件：
     ...
     - `resources/templates/pre-commit`、`pre-push`、`commit-msg`（v3.5.0 新增：归因闸门）-> `.githooks/`，并 `chmod +x`；指导执行 `git config core.hooksPath .githooks`
...
`resources/DEVELOPMENT_STANDARDS.md`、`resources/AGENTS.md` 与方法论层（`resources/METHODOLOGY.md` + `resources/methodologies/`）应随规范正文迭代更新（当前携带版本 v3.7.0，见规范页脚）；`resources/templates/agent-gate.sh` 与 `tests/run-tests.sh` 必须同步演进--改脚本必须先跑通 `tests/run-tests.sh` 再发布（§2.17.4）。**改规范正文 / 模板 / README 后必须另跑通 `tests/audit-docs-consistency.sh`**（文档一致性机器审计：版本链、关键词落点矩阵、§3↔§4 清单同源、编号体系收录、占位符 vs A 层断言防恒真、骨架 vs A 层关键词、00 管线件落点、bugfix 双登记 9+1 项对齐、锚点章节存在性、README 树↔磁盘、升级日志排序）--文档完整性由方法与机器保证，不由轮数保证。**适用范围分层**：本脚本属规范源层自检（仅本仓库使用，**不入**上方复制清单）；通用层（`agent-gate.sh`/`run-tests.sh`/`check-standards-compliance.sh`）审计目标仓库的变更产物，两层不可互替。脚本断言分 PART A（永久结构不变量）与 PART B（版本快照落点，规范升级时随 §2.14 日志更新），防脚本腐烂。每次升级本 Skill 内的规范版本后，已经接入过的项目**不会自动更新**，需要用户再次调用本 Skill 走"检测已有文件 -> 展示版本差异 -> 询问是否升级"的流程（v3.3.0+ 升级须额外复制 `METHODOLOGY.md`、`methodologies/`（v3.4.0 含 `state-trigger-audit.md`）与 `bugfix-log.md` 到目标仓库 `docs/`；v3.5.0+ 升级须额外复制 `commit-msg` 模板到 `.githooks/` 并更新 `pre-push` 与 `scripts/agent-gate`；v3.6.0+ 升级须额外复制缺陷文档组模板（`bug-diagnosis.md`/`bug-impact.md`/`bug-test-plan.md`）到目标仓库 `docs/bugs/_templates/`；v3.7.0+ 升级须将缺陷模板扩为六件套（额外复制 `bug-matrix.md`/`bug-config.md`/`bug-tasks.md`）、额外复制编码记录模板 `coding-record.md`（变更起编时落 `docs/changes/<变更号>/04.5-coding-record.md`）并同步更新 `scripts/agent-gate` 与 `tests/run-tests.sh`）。
