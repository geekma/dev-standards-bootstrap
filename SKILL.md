...
3. **写入核心文件**（无冲突时直接写入）：
   ...
   - 复制 `resources/templates/bug-diagnosis.md`、`bug-impact.md`、`bug-test-plan.md` -> 目标仓库 `docs/bugs/_templates/`（v3.6.0 新增：缺陷文档组三件套模板——改 Bug 一律在 `docs/bugs/<BUG-xxx>/` 新起一组三件套并经 `00-governance.json` 的 `bug_ref` 字段与变更关联，只增不改，§2.15 硬性规则 4）
...
`resources/DEVELOPMENT_STANDARDS.md`、`resources/AGENTS.md` 与方法论层（`resources/METHODOLOGY.md` + `resources/methodologies/`）应随规范正文迭代更新（当前携带版本 v3.6.0，见规范页脚）；`resources/templates/agent-gate.sh` 与 `tests/run-tests.sh` 必须同步演进--改脚本必须先跑通 `tests/run-tests.sh` 再发布（§2.17.4）。**改规范正文 / 模板 / README 后必须另跑通 `tests/audit-docs-consistency.sh`**（文档一致性机器审计：版本链、关键词落点矩阵、§3↔§4 清单同源、编号体系收录、占位符 vs A 层断言防恒真、骨架 vs A 层关键词、00 管线件落点、bugfix 双登记 8+1 项对齐、锚点章节存在性、README 树↔磁盘、升级日志排序）--文档完整性由方法与机器保证，不由轮数保证。**适用范围分层**：本脚本属规范源层自检（仅本仓库使用，**不入**上方复制清单）；通用层（`agent-gate.sh`/`run-tests.sh`/`check-standards-compliance.sh`）审计目标仓库的变更产物，两层不可互替。脚本断言分 PART A（永久结构不变量）与 PART B（版本快照落点，规范升级时随 §2.14 日志更新），防脚本腐烂。每次升级本 Skill 内的规范版本后，已经接入过的项目**不会自动更新**，需要用户再次调用本 Skill 走"检测已有文件 -> 展示版本差异 -> 询问是否升级"的流程（v3.3.0+ 升级须额外复制 `METHODOLOGY.md`、`methodologies/`（v3.4.0 含 `state-trigger-audit.md`）与 `bugfix-log.md` 到目标仓库 `docs/`；v3.5.0+ 升级须额外复制 `commit-msg` 模板到 `.githooks/` 并更新 `pre-push` 与 `scripts/agent-gate`；v3.6.0+ 升级须额外复制缺陷文档组三件套模板（`bug-diagnosis.md`/`bug-impact.md`/`bug-test-plan.md`）到目标仓库 `docs/bugs/_templates/`）。
