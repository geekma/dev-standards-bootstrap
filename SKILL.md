...
- **同步范围**：`resources/DEVELOPMENT_STANDARDS.md`、`resources/AGENTS.md` 与方法论层（`resources/METHODOLOGY.md` + `resources/methodologies/`）应随规范正文迭代更新（当前携带版本 v3.9.0，见规范页脚）；`resources/templates/agent-gate.sh` 与 `tests/run-tests.sh` 必须同步演进——改脚本必须先跑通 `tests/run-tests.sh` 再发布（§2.17.4）；`scripts/bootstrap.sh` 的清单与 SKILL.md 步骤 3-4 同步维护。
