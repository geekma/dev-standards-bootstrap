...
`A11` 断言 `resources/DEVELOPMENT_STANDARDS.md` 不超过一个硬上界（当前 **164 KB / 167,936 字节**）。它不是"防内容变多"，而是**防无序回弹**——上界一旦失效，规范就会无声地膨胀下去。

**重校准是设计内的动作，不是破例**，但必须满足两个条件：

v3.43.0 重校准记录（CHG-053）：156KB 上界被 §2.17.2b 缺陷发现入口（三信号双通道 + 频控 + 自主权，160,160 字节）吃到负值 → 按 KiB 步进（+8KB，跨 4KB 档取整）重设 **164KB**（余量约 7.5KB），有 CHANGELOG v3.43.0 行对应。

1. **有 CHANGELOG 条目对应**（新条款必须登记）；
2. **先压缩再抬高**：把重复条款合并、把机制细节下沉到脚本头部注释（渐进披露），能省多少先省多少。

v3.31.0 的重校准记录（前次，供下次参照；最近一次为上方 v3.43.0 记录——当时 §5.1「当前」句漏同步 144KB 口径，CHG-057 修正）：§2.1 规则 10（角色单一职责）+ §5 反模式行把 140 KB 上界的余量吃到 **-220 字节**（先压缩两处措辞仍压线），按 **KiB 步进（+4 KB）**重设到 144 KB。
...
1. **每新增一条规范条款**（REQ/门禁/机器断言/模板必填件），同一版本内**对等淘汰**一条既有条款，或在 09「重要上下文」登记**预算豁免理由**（为何本条不可裁剪既有条款）。
2. **淘汰必须有痕**：规范内「废止」字样同行必须携带被替代版本指向（`v3.x`）——通用层审计 **G10** 机校此形态（缺指向 = 只废未清，FAIL）；被替代条款的机器断言与模板引用**同版本清扫**，禁止留死断言。
3. **优先减法序**：合并重复条款 > 机制细节下沉脚本注释（渐进披露）> 删除未命中条款 > 抬上界。前三条走不通才允许上界重校准（§5.1 两条件仍适用）。
4. 预算执行情况为 **B 层**核对项（评审角色逐版本核对 CHANGELOG 条目的加/减平衡），不做条款计数机校（M2 未命中即启用 = 过度设计）。

> v3.45.0 注：断言层的预算与退役流程见 **§5.3 断言生命周期**（soft 桶 + 近两版捕获评审阶梯）；本节保持条款级，不与断言级混算。

### 5.3 断言生命周期（v3.45.0 CHG-055，Top3 装牙）

audit（源层）与 golden（通用层）断言只进不出 = 维护税复利。本节给断言装退役通道：

1. **四类标注**：新断言入册时在调用点注释标类型——〔结构〕存在性/机校逻辑、〔措辞〕关键词落点、〔行为〕运行时计算、〔计数〕表格/清单行数。
2. **近两版捕获评审**：每版本交付评审时核对——某措辞/计数类断言**近两版未捕获过真问题**（拦到漂移/缺陷）→ 列候选。
3. **阶梯处置**：候选断言先降 soft（audit 走 `report_soft`，CI warn 不阻断）观察一版；仍无捕获 → 退役（删除 + SLOG 行 + 09 登记，G10 式有痕）。结构/行为类不适用本阶梯（存在性防线不可降）。
4. **soft 桶滥用防线**：每次 `report_soft` 新调用必须在变更 09 登记（名称 + 理由），评审核对；禁止把硬断言静默改 soft。当前 soft 面：A3 数字闭环 / A3c 同步 / A10 基线（仅 `AGENT_GUARD_AUDIT_SOFT=1` CI 模式，本地恒硬）。
5. 自指断言（grep 断言所在脚本自身的字符串）无值守价值，入册即列退役候选。
...
| 版本 | 升级后需要补什么 |
|---|---|
| **v3.51.0** | **安装清单全文件保留（CHG-063，用户指令追问→通用化）**：`.dev-standards-manifest`（cksum 基线）+--upgrade 逐文件归因（版本漂移=updated/用户改=kept--force 越/legacy=kept+采纳提示）；四层叠加（LIVE_SKIP/AGENTS 合并/清单/CONFLICT）；golden 485→492+A34×2（393→395）。**`--upgrade` 带走 bootstrap+规范**。**回填义务：无**；A11 165,247B（DS 零改动，余量 2,689B） |
| **v3.50.0** | **安装/升级用户内容保留（CHG-062，用户指令）**：AGENTS.md 托管标记块+bootstrap `merge_managed`——agents-md 键 merge-or-keep（有标记只换托管区用户区块保留；无标记差异 kept，--force 不越，删除重装刷新）；通用 conflict/--force 不变；golden 480→485+A33×2（391→393）。**`--upgrade` 带走 bootstrap/install/规范**。**回填义务：无**；A11 165,247B（余量 2,689B，DS 零改动） |
| **v3.49.0** | **版本载体全链清单条款（CHG-061，CHG-059 可否决点③批准）**：DS §2.14 新增载体全链唯一权威条款（页脚×9+元数据×2+README×2×3 处+升级记录×2，入链/出链同版本修订）+注册表 R42 universal 行（41→42）+§5.2 对等减法=§2.16.5「标准升级」行退役+die 码域 `[0-9]+`（T30×3+T41，P3-1 收口）；A32×2（源层 389→391）、T39 计数 42。**`--upgrade` 带走规范/注册表**。**回填义务：无**。golden 480 不变，源层断言 389→391（基线 updater 同步）；A11 165,247B（余量 2,689B） |
| **v3.48.1** | **gate 检查码重编号（CHG-060，CHG-058 P3-6 遗留）**：E15→E80（stop 轨 02 延伸发现）、E16→E81（begin bug_ref 轨 01-diagnosis 延伸发现）、签名检查 E52→E82（执行期延伸发现并入；stub E52 保留）——检查逻辑零变化；gate die 码唯一性 golden T41（479→480）；fresh A/B 实测数据落账（FU-106⑦ 全闭合，CHG-060 09 节引用 CHG-059）。**`--upgrade` 带走 agent-gate/run-tests**。**回填义务：无**。golden 479→480，源层断言 389 不变（基线 388） |
| **v3.48.0** | **缺陷/变更闭环三落点（CHG-058，CHG-057 裁定方案 A）**：延伸发现即时落盘（阶段 2/6 节+§2.12 发现即登记+gate E15/E16）+历史相似检索步（阶段 2/6 节+entry 骨架，无命中写"未命中（检索词）"防假检索）+历史关联必填（七要素⑦+bugfix-log 模板字段+§2.16.5 勘误引用原号+G4 增量执法：最新 BUG 条目须含历史相似，存量不追）。**执法面**：golden +5、源层 +A30×6+A31×4（G10 枚举面 pin=README/SKILL G10 漏更复发防线）。**`--upgrade` 带走规范/agent-gate/audit/bugfix-log 模板/entry 骨架**。**回填义务：无**。A11 164,797B（余量 3,139B）。golden 474→479，源层断言 379→389（基线 388） |
| **v3.47.0** | **注册表类型扩面（CHG-057 批 2，Top1 规范倒置第二步）**：CLAUSE_REGISTRY 28→41 行——L1-standards 全集 +6（§1.1/§1.2/§2.5/§2.14/§2.16.5/§4）、L2-architecture 全集 +5（§2.7/§2.8/§2.9/§2.10/§2.13）、L3-critical +2（§2.7.3/§0.5.1），骨架行 R21-R25 保留，锚点写行前 grep 验真=1；L1/L2/L3 变更取包从 1-3 条款骨架变为全集。golden T39 计数 28→41+五类型最小集下限、T40 fixture 扩三类型正常态。§5.1「当前」上界句 144→164KB 口径修正（v3.43.0 重校漏同步，A28 防复发）。A11 评估：DS 163,218B/上界 167,936B 余量 4,718B——本批 DS 正文零改动，减法留批 3+（A/B 实测数据裁决）。§5.2 预算：+3 REQ 对等减法=L1/L2/L3 手工锚点对照阅读路径被注册表包替代延续。**`--upgrade` 无新分发物**（注册表为既有分发件数据行增量）。**回填义务：无**。golden 463→474（T39+5/T40+6），源层断言 376→379（A28+A29×2 SKILL 清单枚举面，基线 378） |
| **v3.46.0** | **条款注册表与阅读包（CHG-056 批 1，Top1 规范倒置第一步）**：新增 `CLAUSE_REGISTRY.md`（条款机器索引层：Rxx 行 ID + `文件名::唯一锚点串` + 类型封闭词表 + 生命周期四类；DS 仍是正文唯一权威）与 `generate-reading-pack.sh`（按类型生成阅读包，锚点 fail-closed 防漂移，`--verify` 可入 CI）；DS §2.17.2c 新条款 + AGENTS 锚点表路由行。**`--upgrade` 带走 CLAUSE_REGISTRY.md（→ `<docs>/`）+ generate-reading-pack.sh（→ `<scripts>/`）+ 规范/AGENTS/agent-gate 豁免**。**回填义务：无**——不跑生成命令=零行为变化；A11 减重留批 2+（本版 DS 仅增 §2.17.2c）。golden 463，源层断言 376（基线 375）。 |
| **v3.45.0** | **断言预算制（CHG-055，Top3）**：audit soft 桶（`AGENT_GUARD_AUDIT_SOFT`，A3/A3c/A10 CI 降 warn）+ 断言退役净减 3（双形状/自指 pin）+ MAINTAINER §5.3 断言生命周期（近两版捕获评审→降 soft→退役阶梯）+ DS §2.1 规则 8 歧义分级裁定（bus factor 1 解法）。**`--upgrade` 带走规范/AGENTS/MAINTAINER**（audit/ci.yml/run-tests 源层件）。**回填义务：无**。golden 441，源层断言 366（基线 365）。 |
