...
1. 在 `docs/changes/<变更号>/` 创建并完成 `00-intent.md`、`00-governance.json`、`00.5-communication.md`、`01-spec.md`、`02-code-impact-analysis.md`、`03-modification-plan.md`、`03.5-tasks.md`、`04-test-scripts.md`（**入口骨架可由 `scripts/new-change <变更号> --risk Lx` 生成，v3.38.0；v3.52.0 起两段式——确认记录在位后才补齐 01-spec 起五件，`--allow-unconfirmed` 供自动化入口显式越过**；骨架不豁免填写，PENDING 执行主体被 begin 拒绝）；`00-intent.md` 是变更管线入口（问题/预期结果/开放问题/约束等，§2.17；`begin` 硬校验"预期结果"与"开放问题"两节）；**`00.5-communication.md` 是沟通先行门产物（深挖结论/影响/风险/≥2 候选方案取舍/测试思路/用户整体确认记录，§2.17.2d；begin 硬校确认记录非空，GATE-E83；L0 可单行声明豁免）——确认记录落盘前禁止生成后续产物正文与源码编辑**，02 是先分析后方案的强制前提（业务/技术/风险三维 + 回滚策略，§2.5 阶段 2），JSON 必须声明风险等级和执行主体（含规格/文档作者 `spec_author`，§2.1 规则 10），L2/L3 的开发、测试、Review 主体必须不同。**同一天多个 L0/L1 默认共落 `docs/changes/BATCH-YYYYMMDD/`（`## <变更号>` 锚点分节；L2/L3 独立目录；细则 §1.1）**。涉及需求或功能行为的变更，交付前须回填功能级 RTVM 矩阵（REQ→DES→TASK→TC + 验证状态，门禁 4）：changelog 引用的每个 REQ 都必须有矩阵回填行，缺失即未闭环（stop/CI 与 audit G5 均拦截）。
...
任务开始时，编排者会告知你在本次任务中扮演的角色（需求 Agent / 架构-计划 Agent / 开发 Agent / 测试 Agent / Review Agent，见规范 §2.1）。**你只能完成本角色允许的工作，不得越权完成其他角色的验证与签署**——即使你有能力做到。这是本规范最核心的约束，不因效率考虑而放松。**同一变更内不得兼任多个角色**（规格/文档作者、实现、测试、评审四主体互异，L0/L1 最低线=评审独立且作者≠评审，§2.1 规则 10）。若被细分指派为 §2.2 某专家（业务/行业/技术/架构/项目管理/安全/性能/测试/整体验收），还须满足该专家的评审动作、上下文贴近契约与署名要求（专家是角色的细分执行主体，签署权映射回 §0.5.2 既有角色列）。**分阶段评审留痕（v3.53.0 硬指令）**：阶段 1-5 的 B 层评审**必须逐阶段派发独立子代理**（输入契约 §2.2）并在 `07-review-report.md`「分阶段评审」节留痕；L0/L1 可单行声明合并评——**禁止"只做阶段 8 终审"的收敛执行**。
