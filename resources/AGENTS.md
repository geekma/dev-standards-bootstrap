...
| 只是改 bug | §2.5 阶段 6（含 `docs/bugfix-log.md` 双登记 + 根因表「同族推演」行 + 「Bug 修复回填清单」；v3.35.0 三条款：诊断前必读总册相关册、TC 不覆盖必须新增防回归 TC、六件套按天入批 `docs/bugs/BATCH-YYYYMMDD/`（v3.36.0 扁平：同名文件 + `## BUG-xxx` 锚点，当天同文件追加）；**v3.40.0 P3 缺陷轻量通道：`01-diagnosis.md` 行首 `severity: P3`（文案类）免 02/05/06 三件，未声明=全六件**）+ `docs/methodologies/state-trigger-audit.md`（同族推演与反模式细则）+ §0 门禁 |
| 交付前文档互证（CI/本地均可跑） | `bash tests/audit-docs-consistency.sh`（G1 版本链 / G2 编号连续 / G3 归档清单↔§3 同源 / G4 bugfix 双登记互证 / G5 RTVM 回填一致 / G6 §4 必填节 / G7 变更批次自洽 / G8 六件套存在性 / **G9 项目总册存在+自证+回填清单+功能目录表外 reviews/ 拦截（§1.3 第 6 条，v3.37.1）** / **G10 废弃条款 sweep——废止标记须带 v3.x 被替代指向（减法机制，v3.40.0）** / A20 变更目录盖章互证 / A21 六件套盖章，v3.35.0；失败项即 §2.14 回填清单，§2.13.4） |
...
任务开始时，编排者会告知你在本次任务中扮演的角色（需求 Agent / 架构-计划 Agent / 开发 Agent / 测试 Agent / Review Agent，见规范 §2.1）。**你只能完成本角色允许的工作，不得越权完成其他角色的验证与签署**——即使你有能力做到。这是本规范最核心的约束，不因效率考虑而放松。**同一变更内不得兼任多个角色**（规格/文档作者、实现、测试、评审四主体互异，L0/L1 最低线=评审独立且作者≠评审，§2.1 规则 10）。若被细分指派为 §2.2 某专家（业务/行业/技术/架构/项目管理/安全/性能/测试/整体验收），还须满足该专家的评审动作、上下文贴近契约与署名要求（专家是角色的细分执行主体，签署权映射回 §0.5.2 既有角色列）。
