...
with open(path, "w") as fh:
    ...
    fh.write("\n")
PY
  # 生成后验证：JSON 必须可解析，三条接线必须就位
  python3 - "$f" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
h = d.get("hooks", {})
cmds = [hk.get("command") for st in h.values() for e in st for hk in e.get("hooks", [])]
for want in ("scripts/session-gate.sh start", "scripts/agent-gate --stage pre-write", "scripts/agent-gate --stage stop"):
    if want not in cmds:
        sys.exit(f"missing hook after merge: {want}")
PY
  ok "claude -> $f merged + verified (SessionStart audit / pre-write / stop)"
}
...
// dev-standards-gate.js — generated by scripts/install-hook-adapter (v3.28.0).
// Regenerated on reinstall; manual edits will be overwritten.
// session.created -> scripts/session-gate.sh start  (audit: 存量红灯亮给本会话)
// session.idle    -> scripts/session-gate.sh idle   (gate --stage stop 等价检查落盘)
export const DevStandardsGate = async ({ client, $, directory }) => {
  const run = async (mode) => {
    try {
      ...
      for (const line of out.split("\n")) {
        ...
        const level = line.includes("GATE RED") ? "warn" : "info"
        try {
  ...
  return {
    event: async ({ event }) => {
      if (event.type === "session.created") await run("start")
      else if (event.type === "session.idle") await run("idle")
    },
  }
}
JS
