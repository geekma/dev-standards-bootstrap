...
[![Standards Version](https://img.shields.io/badge/Standards-v3.22.0-green.svg)](resources/DEVELOPMENT_STANDARDS.md)
...
| **Golden-Case Self-Tests** | `tests/run-tests.sh` regression-tests the gate and the installer (292 golden-case assertions) in throwaway git repos — bash + git only (§2.17.4) |
...
- **Same-day change batches (v3.18.0)**: several **L0/L1** changes from the **same day** may share one `<docs>/changes/BATCH-YYYYMMDD/` directory instead of one directory each. Only the directory is relaxed — artifact filenames are unchanged, bundled changes are told apart by a `## <change-id>` section anchor, and the batch's change set is read from the authoritative roster in `00-governance.json` (never inferred from headings). The risk ceiling is not relaxed: **L2/L3 must live in their own directory**, and the gate refuses an L2/L3 record found inside a batch (per the governance roster).
...
└── resources/
    ...
    ├── DEVELOPMENT_STANDARDS.md             # Full standards document v3.22.0 (copied to docs/)
...
**Standards Version:** v3.22.0 | **Last Updated:** 2026-09-16 | **Maintainer:** [geekma](https://x.com/geekma) | **Email:** geekma@gmail.com
