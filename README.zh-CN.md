...
[![Standards Version](https://img.shields.io/badge/规范版本-v3.40.0-green.svg)](resources/DEVELOPMENT_STANDARDS.md)
...
| **golden case 自测试** | `tests/run-tests.sh` 在临时 Git 仓库里回归测试门禁与安装器（395 项 golden-case 断言）——只需 bash + git（§2.17.4） |
...
└── resources/
    ...
    ├── DEVELOPMENT_STANDARDS.md             # 完整规范文档 v3.40.0（复制到 docs/）
    ...
    └── templates/
        ...
        ├── entry                             # 变更入口骨架（v3.40.0）：begin 必检七件模板（→ docs/templates/entry/）
        ├── new-change.sh                   # 变更入口脚手架（v3.40.0）：专用目录/当日批次自动判定（→ scripts/new-change）
...
**规范版本：** v3.40.0 | **最后更新：** 2026-09-22 | **维护者：** [geekma](https://x.com/geekma) | **邮箱：** geekma@gmail.com
