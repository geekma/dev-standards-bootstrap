...
- **缺陷环节三条款（规范 v3.35.0，BUG-005）**：诊断前必读总册相关册；修复落点不被既有 TC 覆盖必须新增防回归 TC；缺陷六件套按天入批（v3.36.0 扁平化为 `docs/bugs/BATCH-YYYYMMDD/` 同名文件 + `## BUG-xxx` 锚点，嵌套形态历史保留）。
...
- audit 新增 G9（总册存在/自证/评审配对/回填清单+功能目录表外 reviews/ 拦截）；源层审计新增 A25，A11 上界重校准 144KB→156KB；golden 310→324，源层断言 282→309。
